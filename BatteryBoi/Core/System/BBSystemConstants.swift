...
struct SystemDeviceObject: Hashable, Equatable, Identifiable {
    ...
    init?(_ device: Devices) {
        if let id = device.id, let model = device.model {
            ...
            self.profile = .init(model: model, subtype: device.subtype, vendor: device.vendor ?? "", apperance: device.apperance, findmy: device.findmy)
            self.synced = true
            ...
            self.refreshed = device.refreshed_on
